In this section, we shall prepare some facts on the [logarithmetic] geometry of hyperbolic polycurves, which will be used as basic ingredients in the subsequent sections. In particular, we shall verify that the stable compactification of a hyperbolic polycurve admits a natural log structure, which is compatible with a certain stratification. We then discuss some general properties of log schemes whose characteristics are, in some sense, \'{e}tale locally constant. Finally, we introduce the notion of (pro-$\ell$) decomposition groups and inertia groups, where $\ell$ is a prime number which is invertible on the base scheme, associated to the strata of the stable compactification of a hyperbolic polycurve, and verify some properties of these groups.
...
\begin{defn}
  ...
  \begin{enumerate}
    ...
    \end{itemize}
    \item Let $x$ be a stratum point of $\model{X}_n$. Let $x_i$ be the image of $x$ in $\model{X}_i$ for each $i = 0, \ldots, n$. We shall refer to as \emph{VCNGS-signature} of $x$ the function $\sign{x} \colon \{0, \ldots, n\} \to \{\textV, \textC, \textN, \textG, \textS\}$ defined as follows:
    \begin{itemize}
...
\begin{defn}
  ...
  \begin{enumerate}
    ...
    \]
    where $H$ is the kernel of the natural morphism $\Ker(\pi_1(A^\log, a^\log) \to G_k) \to \IPRDG{\ell}{A}$ (respectively, $\Ker(\pi_1(A, a) \to G_k) \to \IPRBDG{\ell}{A}$). Then we shall refer to the profinite group $\pi_1(A^\log, a^\log)/H$ (respectively, $\pi_1(A, a)/H$) arising from the diagram as the \emph{$\ell$-tamely reduced geometrically pro-$\ell$ decomposition group} of $A$ (respectively, \emph{$\ell$-tamely reduced geometrically pro-$\ell$ base-decomposition group} of $A$), and we shall write $\lTRGPRDG{\ell}{A}$ (respectively, $\lTRGPRBDG{\ell}{A}$) for the $\ell$-tamely reduced geometrically pro-$\ell$ decomposition group of $A$. We abbreviate the term ``$\ell$-tamely reduced geometrically pro-$\ell$ decomposition group'' (respectively, ``$\ell$-tamely reduced geometrically pro-$\ell$ base-decomposition group'') to ``\emph{$\ell$-TR geometrically pro-$\ell$ decomposition group}'' (respectively, ``\emph{$\ell$-TR geometrically pro-$\ell$ base-decomposition group}'').
    ...
    \[
      1 \to \IPRDG{\ell}{A} \to \pi_1(A^\log, a^\log)/H \to G_k \to 1,
    \]
    where $H$ is the kernel of the natural morphism $\Ker(\pi_1(A^\log, a^\log) \to G_k) \to \IPRDG{\ell}{A}$. We shall refer to the profinite group $\pi_1(A^\log, a^\log)/H$ as the \emph{$\ell$-tamely reduced geometrically pro-$\ell$ decomposition group} of $A$, and we shall write $\lTRGPRDG{\ell}{A}$ for the $\ell$-tamely reduced geometrically pro-$\ell$ decomposition group of $A$. We abbreviate the term ``$\ell$-tamely reduced geometrically pro-$\ell$ decomposition group'' to ``\emph{$\ell$-TR geometrically pro-$\ell$ decomposition group}''.
...
\begin{prop}[Decomposition groups and inertia groups associated to strata, certain exact sequences]\label{DecompositionGroupsAndInertiaGroupsAssociatedToStrataCertainExactSequences}
  ...
  \begin{enumerate}
    ...
    \begin{center}
      \begin{tikzpicture}[auto]
        ...
        \node (B4) at (6.0, 1.2) {$\IPRBDG{\ell}{A}$};
...
\begin{proof}
  ...
  We first observe, in light of the definition of the stratification, that the morphism $A^\log \to \overline{A}^\log$ is a hyperbolic curve over $\overline{A}^\log$ if $\sign{\eta_A}(n) = \textV$. Moreover, in that case, there is a compactification $A^\log_\cpt$ of $A^\log$ as a hyperbolic curve which is obtained as an irreducible component of $\model{X}^\log_n \times_{\model{X}^\log_{n - 1}} \overline{A}^\log$. Now, we can easily verify that the right exact portion of the assertion (iv) of the case $\sign{\eta_A}(n) = \textV$ (respectively, $\sign{\eta_A}(n) = \textC$; $\sign{\eta_A}(n) = \textN$) follows from \cite[Theorem 2]{Hoshi1} (respectively, Theorem \ref{ExactSequenceOfLogFundamentalGroupsOfEtaleLocallyLogConstantLogSchemes}; multiple applications of Theorem \ref{ExactSequenceOfLogFundamentalGroupsOfEtaleLocallyLogConstantLogSchemes}). Now, the assertion (v) follows immediately from the various definitions involved.
  ...
  \begin{center}
    \begin{tikzpicture}[auto]
      \node (A1) at (1.5, 1.2) {$1$};
      \node (B1) at (1.5, 0) {$1$};
      \node (A2) at (3.5, 1.2) {$\pi_1(A_\text{fib.}^\log, a^\log)^\ell$};
      \node (B2) at (3.5, 0) {$\GFG{X_n/X_{n - 1}}^\ell$};
      \node (A3) at (6.0, 1.2) {$\GPRDG{\ell}{A}$};
      \node (B3) at (6.0, 0) {$\GFG{X_n}^\ell$};
      \node (A4) at (8.0, 1.2) {$\GPRDG{\ell}{\overline{A}}$};
      \node (B4) at (8.0, 0) {$\GFG{X_{n - 1}}^\ell$};
      \node (A5) at (10.0, 1.2) {$1$};
      \node (B5) at (10.0, 0) {$1.$};
      \draw[->] (A1) to node {} (A2);
      \draw[->] (B1) to node {} (B2);
      \draw[->] (A2) to node {} (A3);
      ...
      \draw[->] (A2) to node {} (B2);
      \draw[->] (A3) to node {} (B3);
      \draw[->] (A4) to node {\scriptsize inj.} (B4);
    \end{tikzpicture}
  \end{center}
  Here, the exactness of the top row, i.e., the content of assertion (iv) (respectively, the injectivity of the middle vertical arrow) follows immediately from the assertion (v) (respectively, the assertion (v)). Assertion (i) follows immediately from the injectivity of the morphism $\GPRDG{\ell}{A} \to \GFG{X}^\ell$.

  To verify the assertion (ii), it suffices to verify the exactness of the top row of the commutative diagram. However, the right exactness of the row follows immediately from Theorem \ref{ExactSequenceOfLogFundamentalGroupsOfEtaleLocallyLogConstantLogSchemes}. Therefore, it suffices to verify the injectivity of the morphism $\PRIG{\ell}{A} \to \GPRDG{\ell}{A}$. 
  
  For the case $\sign{\eta_A}(n) = \textV$, the injectivity of the morphism $\PRIG{\ell}{A} \to \GPRDG{\ell}{A}$ follows immediately from the injectivity of the morphism $\PRIG{\ell}{A} \to \GPRDG{\ell}{\overline{A}}$ (which is a consequence of the assertion (ii) for smaller values of $n$). For the case $\sign{\eta_A}(n) = \textC$ or $\textN$, since the profinite group $\PRIG{\ell}{A}$ can be naturally identified with the pro-$\ell$ fundamental group of the log geometric fiber $A_\text{fib.}^\log$, the injectivity of the morphism $\PRIG{\ell}{A} \to \GPRDG{\ell}{A}$ follows immediately from the assertion (iv).

  To verify the assertion (iii), it suffices to verify the exactness of the middle row of the commutative diagram. However, the right exactness of the row follows immediately from Theorem \ref{ExactSequenceOfLogFundamentalGroupsOfEtaleLocallyLogConstantLogSchemes}. Therefore, it suffices to verify the injectivity of the morphism $\PRIG{\ell}{A} \to \IPRDG{\ell}{A}$.

  For the case $\sign{\eta_A}(n) = \textV$, the injectivity of the morphism $\PRIG{\ell}{A} \to \IPRDG{\ell}{A}$ follows immediately from the injectivity of the morphism $\PRIG{\ell}{A} \to \IPRDG{\ell}{\overline{A}}$ (which is a consequence of the assertion (iii) for smaller values of $n$). For the case $\sign{\eta_A}(n) = \textC$ or $\textN$, since the profinite group $\PRIG{\ell}{A}$ can be naturally identified with the pro-$\ell$ fundamental group of the log geometric fiber $A_\text{fib.}^\log$, the injectivity of the morphism $\PRIG{\ell}{A} \to \GPRDG{\ell}{A}$ follows immediately from the assertion (iv) and the surjectivity of the morphism $\IPRDG{\ell}{A} \to (I_k/P_k)^\ell$ (which is a consequence of the geometric connectedness of the morphism $A \to \Spec\, k$ and the surjective portion of Theorem \ref{ExactSequenceOfLogFundamentalGroupsOfEtaleLocallyLogConstantLogSchemes}).

  Assertion (vi) and (vii) follow immediately from the various computations of the various log structures involved (cf. Lemma \ref{DescriptionOfLogStructureOfStableCompactification}). This completes the proof of the proposition. 
\end{proof}

\begin{defn}
  Let $K$ be a field. Let $\ell$ be a prime number. We shall say that $K$ is \emph{$\ell$-cyclotomically full} if the $\ell$-adic cyclotomic character $\chi_{K, \ell} \colon G_K \to \Int_\ell^\times$ of $K$ has an open image.
\end{defn}

\begin{rem}
  Let $K$ be a complete discrete valuation field, and let $k$ be the residue field of $K$. Let $\ell$ be a prime number which is invertible in $\mathcal{O}_K$. Then $K$ is $\ell$-cyclotomically full if and only if $k$ is $\ell$-cyclotomically full. In particular, it follows from the fact that the $\ell$-adic cyclotomic character $\chi_{K, \ell} \colon G_K \to \Int_\ell^\times$ of $K$ factors through the specialization morphism $G_K \to G_k$, which is (obviously) surjective.
\end{rem}

\begin{prop}[Computation of the centers of various decomposition groups]\label{ComputationOfCentersOfVariousDecompositionGroups}
  Let $K$ be a complete discrete valuation field, and let $X = X_n \to X_{n - 1} \to \cdots \to X_1 \to X_0 = \Spec \, K$ be a hyperbolic polycurve over $K$. Let $\ell$ be a prime number which is invertible in $\mathcal{O}_K$. Assume that $X$ admits a compactified stable model $\model{X} = \model{X}_n \to \model{X}_{n - 1} \to \cdots \to \model{X}_1 \to \model{X}_0 = \Spec \, \mathcal{O}_K$ with respect to its parametrization, and that $X$ is inertia pro-$\ell$-exact and graphically inert. Let $\mathfrak{A}$ be the stratification of $\model{X}_n$ obtained by applying the construction in Lemma \ref{StratificationOfStableCompactification} to the hyperbolic polycurve $X_n$. Let $A$ be a stratum of $\mathfrak{A}$, let $\overline{A}$ be the image of $A$ in $\model{X}_{n - 1}$, and let $\eta_A$ be the generic point of $A$. We identify $\PRIG{\ell}{A}$ with its images in $\GPRDG{\ell}{A}$, $\IPRDG{\ell}{A}$, and $\lTRGPRDG{\ell}{A}$, as in Proposition \ref{DecompositionGroupsAndInertiaGroupsAssociatedToStrataCertainExactSequences}. Then the following assertions hold.
  \begin{enumerate}
    \item Assume that $\sign{\eta_A}(0) = \textG$. Then the equality $C(\GPRDG{\ell}{A}) = \PRIG{\ell}{A}$ holds.
    \item Assume that $\sign{\eta_A}(0) = \textS$. Then the equality $C(\IPRDG{\ell}{A}) = \GPRIG{\ell}{A}$ holds.
    \item The inclusion $\PRIG{\ell}{A} \subset C(\IPRDG{\ell}{A})$ holds.
    \item Assume that $\sign{\eta_A}(0) = \textG$, and that $\lTRGPRBDG{\ell}{A}$ is slim. Then the inequality $C(\lTRGPRDG{\ell}{A}) \subset \PRIG{\ell}{A}$ holds. Moreover, if $k$ is $\ell$-cyclotomically full, then $\lTRGPRDG{\ell}{A}$ is slim.
    \item Assume that $\sign{\eta_A}(0) = \textS$, and that $\GPRFG{\ell}{A}$ is slim. Then the inequality $C(\lTRGPRDG{\ell}{A}) \subset \PRIG{\ell}{A}$ holds. Moreover, if $k$ is $\ell$-cyclotomically full, then $\GPRFG{\ell}{A}$ is slim.
  \end{enumerate}
\end{prop}
\begin{proof}
  First, assuming that $\sign{\eta_A}(0) = \textG$ (respectively, $\sign{\eta_A}(0) = \textS$), in light of the definition of the stratification, we observe that $A$ has a natural structure of a hyperbolic polycurve over $K$ (respectively, a hyperbolic polycurve over $k$). Moreover, by Proposition \ref{DecompositionGroupsAndInertiaGroupsAssociatedToStrataCertainExactSequences}, (iv), we can easily verify that the polycurve $A$ over $K$ (respectively, the polycurve $A$ over $k$) is geometrically pro-$\ell$-exact. Therefore, by Lemma \ref{SlimnessOfGeometricProEllFundamentalGroupOfGeometricallyProEllExactHyperbolicPolycurves}, $\GFG{A}^\ell$ is slim.

  Assertion (iii) follows immediately from Proposition \ref{DecompositionGroupsAndInertiaGroupsAssociatedToStrataCertainExactSequences}, (vi). To verify the assertion (i) (respectively, the assertion (ii)), therefore, it suffices to verify the inclusion $C(\GPRDG{\ell}{A}) \subset \PRIG{\ell}{A}$ (respectively, the inclusion $C(\IPRDG{\ell}{A}) \subset \GPRIG{\ell}{A}$). However, this follows immediately from the slimness of $\GFG{A}^\ell$ and Proposition \ref{DecompositionGroupsAndInertiaGroupsAssociatedToStrataCertainExactSequences}, (ii) (respectively, Proposition \ref{DecompositionGroupsAndInertiaGroupsAssociatedToStrataCertainExactSequences}, (iii)).

  The former portion of the assertion (iv) (respectively, the assertion (v)) follows from the slimness of $\lTRGPRBDG{\ell}{A}$ (respectively, $\GPRFG{\ell}{A}$) and Proposition \ref{DecompositionGroupsAndInertiaGroupsAssociatedToStrataCertainExactSequences}, (ii) (respectively, Proposition \ref{DecompositionGroupsAndInertiaGroupsAssociatedToStrataCertainExactSequences}, (iii)).

  Finally, we consider the latter portion of the assertion (iv) (respectively, the assertion (v)). Here, in light of Proposition \ref{DecompositionGroupsAndInertiaGroupsAssociatedToStrataCertainExactSequences} (ii) (respectively, Proposition \ref{DecompositionGroupsAndInertiaGroupsAssociatedToStrataCertainExactSequences} (iii)), there is a natural action of $\lTRGPRBDG{\ell}{A}$ (respectively, $\GPRFG{\ell}{A}$) on the pro-$\ell$ inertia group $\PRIG{\ell}{A}$ of $A$. Moreover, it follows immediately from the various definition involved that the center of $\lTRGPRDG{\ell}{A}$ (respectively, $\GPRDG{\ell}{A}$) coincides with the invariant subgroup $(\PRIG{\ell}{A})^{\lTRGPRBDG{\ell}{A}}$ (respectively, $(\PRIG{\ell}{A})^{\GPRFG{\ell}{A}}$). However, since the image of the $\ell$-adic cyclotomic character $\chi_{k, \ell} \colon G_k \to \Int_\ell^\times$ of $k$ is open and the action of $\lTRGPRBDG{\ell}{A}$ (respectively, $\GPRFG{\ell}{A}$) on $\PRIG{\ell}{A}$ is induced by the character $\lTRGPRBDG{\ell}{A} \to \lTAbs{K} \to G_k \to \Int_\ell^\times$ (respectively, the character $\GPRFG{\ell}{A} \to G_k \to \Int_\ell^\times$), it follows from the easy computation that the invariant subgroup $(\PRIG{\ell}{A})^{\lTRGPRBDG{\ell}{A}}$ (respectively, $(\PRIG{\ell}{A})^{\GPRFG{\ell}{A}}$) is trivial. This completes the proof of the proposition.
\end{proof}
