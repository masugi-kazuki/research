  ...
  \begin{enumerate}
    ...
    such that
      \begin{itemize}
        ...
        \item $X$ is isomorphic to $X^\cpt \setminus D$ as a scheme over $S$.
      \end{itemize}
    \item We shall say that $X$ is a \emph{hyperbolic polycurve} (of relative dimension $n$) over $S$ (resp. \emph{stable polycurve} (of relative dimension $n$)) if there exist
      \begin{itemize}
    ...
    such that
      \begin{itemize}
        \item for each $i = 1, \ldots, n$, the morphism $X_i \to X_{i - 1}$ is a hyperbolic curve over $X_{i - 1}$ (resp. a stable curve over $X_{i - 1}$).
      \end{itemize}
...
\begin{defn}
  Let $K$ be a complete discrete valuation field. We shall refer to the profinite group $(G_K/P_K)/(I_K/P_K)^{(\ell')}$ as the \emph{$\ell$-tame absolute Galois group} of $K$, where $(I_K/P_K)^{(\ell')}$ is the maximal prime-to-$\ell$ quotient of $I_K/P_K$. We shall write $\lTA{K}$ for the $\ell$-tame absolute Galois group of $K$.
\end{defn}

\begin{defn}
  Let $K$ be a field, and let $X = X_n \to X_{n - 1} \to \cdots \to X_1 \to X_0 = \Spec \, K$ be a hyperbolic polycurve over $K$. Let $\ell$ be a prime number which is invertible in $\mathcal{O}_K$. Assume that $X$ admits a compactified stable model $\model{X} = \model{X}_n \to \model{X}_{n - 1} \to \cdots \to \model{X}_1 \to \model{X}_0 = \Spec \, \mathcal{O}_K$ with respect to its parametrization, and geometrically pro-$\ell$-exact.
  \begin{enumerate}
    ...
    \[
      1 \to \GFG{X}^\ell \to \TRGPPFG{\ell}{X} \to G_K/P_K \to 1,
    \]
    where $\TRGPPFG{\ell}{X}$ is the pullback of profinite groups $G_K/P_K \times_{\Out(\GFG{X}^\ell)} \Aut(\GFG{X}^\ell)$. We shall refer to $\TRGPPFG{\ell}{X}$ as the \emph{tamely reduced geometrically pro-$\ell$ fundamental group} of $X$.
    \item We shall refer to the pullback of the profinite group $\TRGPPFG{\ell}{X} \times_{G_K/P_K} I_K/P_K$ as the \emph{tamely reduced inertia geometrically pro-$\ell$ fundamental group} of $X$. We shall write $\TRIGPPFG{\ell}{X}$ for the tamely reduced inertia geometrically pro-$\ell$ fundamental group of $X$.
    ...
    \[
      1 \to \GFG{X_i}^\ell \to \TRIGPPFG{\ell}{X_i} \to I_K/P_K \to 1
    \]
    ...
    \item Assume that $X$ is inertia pro-$\ell$-exact. Now we observe that the outer representation $G_K/P_K \to \Out(\GFG{X}^\ell)$, which is induced by the exact sequence in (i) above, factors through the $\ell$-tame absolute Galois group $\lTA{K}$ of $K$. Thus, we obtain an exact sequence of profinite groups
    \[
      1 \to \GFG{X}^\ell \to \lTRGPPFG{\ell}{X} \to \lTA{K} \to 1,
    \]
    where $\lTRGPPFG{\ell}{X}$ is the pullback of profinite groups $\lTA{K} \times_{\Out(\GFG{X}^\ell)} \Aut(\GFG{X}^\ell)$. We shall refer to $\lTRGPPFG{\ell}{X}$ as the \emph{$\ell$-tamely reduced geometrically pro-$\ell$ fundamental group} of $X$. We abbreviate the term ``$\ell$-tamely reduced geometrically pro-$\ell$ fundamental group'' to ``\emph{$\ell$-TR geometrically pro-$\ell$ fundamental group}''.
    \item Assume that $X$ is inertia pro-$\ell$-exact. We shall refer to the pullback of the profinite group $\lTRGPPFG{\ell}{X} \times_{\lTA{K}} (I_K/P_K)^\ell$ as the \emph{inertia pro-$\ell$ fundamental group} of $X$. We shall write $\IPPFG{\ell}{X}$ for the inertia pro-$\ell$ fundamental group of $X$.
  \end{enumerate}
\end{defn}
...
\begin{rem}
  For a hyperbolic polycurve $X = X_n \to X_{n - 1} \to \cdots \to X_1 \to X_0 = \Spec \, K$ over a field $K$, $X$ is graphically inert if and only if for each stratum point $\eta$ of $X_i$ ($0 \leq i \leq n - 1$), the dual graph of the hyperbolic curve $X_{i + 1} \times_{X_i} \eta$ over the field $\kappa(\eta)$ is split. Moreover, this condition is equivalent to the condition that for every stratum $A$ of $\model{X}_i$ ($1 \leq i \leq n$), letting $\overline{A}$ be the image of $A$ in $\model{X}_{i - 1}$, the induced morphism $A \to \overline{A}$ is geometrically connected. Thus, if $X$ is graphically inert and $A$ is a stratum of $\model{X}_i$ ($1 \leq i \leq n$) such that $\sign{\eta_A}(i) = \textC$ (resp. $\sign{\eta_A}(i) = \textN$), where $\eta_A$ is the generic point of $A$, then the morphism $A \to \overline{A}$ is isomorphism (resp. isomorphism).
\end{rem}

\begin{defn}
  Let $K$ be a complete discrete valuation field, $X = X_n \to X_{n - 1} \to \cdots \to X_1 \to X_0 = \Spec \, K$ a hyperbolic polycurve over $K$. Let $K^\ur$ be the maximal unramified extension of $K$, let $k$ be the residue field of $K$, and let $\overline{k}$ be an algebraic closure of $k$. Let $\ell$ be a prime number which is invertible in $\mathcal{O}_K$. Assume that $X$ admits a compactified stable model $\model{X} = \model{X}_n \to \model{X}_{n - 1} \to \cdots \to \model{X}_1 \to \model{X}_0 = \Spec \, \mathcal{O}_K$ with respect to its parametrization, and that $X$ is pro-$\ell$-exact and graphically inert. Let $\mathfrak{A}$ be the stratification of $\model{X}_n$ obtained by applying the construction in Lemma \ref{StratificationOfStableCompactification} to the hyperbolic polycurve $X_n$. Let $A$ be a stratum of $\mathfrak{A}$. Let $\eta_A$ be the generic point of $A$. Let $a^\log$ be a log geometric point of $A^\log$ lying over a geometric point $a$ of $A$. 
  \begin{enumerate}
    \item Assume that $\sign{\eta_A}(0) = \textG$. Then we shall refer to the maximal pro-$\ell$ quotient of the kernel of the morphism $\pi_1(A^\log, a^\log) \to \Gal(K^\ur/K)$ as the \emph{inertia pro-$\ell$ decomposition group} of $A$, and we shall write $\IPRDG{\ell}{A}$ for the inertia pro-$\ell$ decomposition group of $A$.
    \item Assume that $\sign{\eta_A}(0) = \textS$. Write $\overline{s}^\log$ be the log geometric point of $\Spec \, \mathcal{O}_K^\log$ which is image of $a^\log$. Then we shall refer to the maximal pro-$\ell$ quotient of the kernel of the morphism $\pi_1(A^\log, a^\log) \to \pi_1(\Spec \, k^\log, \overline{s}^\log)$ as the \emph{inertia pro-$\ell$ decomposition group} of $A$, and we shall write $\IPRDG{\ell}{A}$ for the inertia pro-$\ell$ decomposition group of $A$.
  \end{enumerate}
\end{defn}


\begin{prop}
  Let $K$ be a complete discrete valuation field, and let $X = X_n \to X_{n - 1} \to \cdots \to X_1 \to X_0 = \Spec \, K$ be a hyperbolic polycurve over $K$. Let $\ell$ be a prime number which is invertible in $\mathcal{O}_K$. Assume that $X$ admits a compactified stable model $\model{X} = \model{X}_n \to \model{X}_{n - 1} \to \cdots \to \model{X}_1 \to \model{X}_0 = \Spec \, \mathcal{O}_K$ with respect to its parametrization, and that $X$ is geometrically pro-$\ell$-exact and graphically inert. Let $\mathfrak{A}$ be the stratification of $\model{X}_n$ obtained by applying the construction in Lemma \ref{StratificationOfStableCompactification} to the hyperbolic polycurve $X_n$. Let $A$ be a stratum of $\mathfrak{A}$, and let $\eta_A$ be the generic point of $A$. Let $a^\log$ be a log geometric point of $A^\log$ lying over a geometric point $a$ of $A$. Write $\overline{a}^\log$ for the image of $a^\log$ in $\overline{A}^\log$, and write $\overline{a}$ for the underlying geometric point of $\overline{a}^\log$. 
\end{prop}
...
\begin{prop}
 ...
 Then the following assertions hold:
  \begin{enumerate}
    \item The (natural) morphism of profinite groups $\pi_1(A^\log, a^\log)^\ell \to \pi_1(\model{X}_n^\log, a^\log)^\ell$ is injective.
    \item   
    \begin{enumerate}
      \item If $\sign{\eta_A}(n) = \textV$, then $A$ is smooth over $\overline{A}$. Moreover, there exists a finite 'etale cover $A'$ of $\overline{A}$ such that, letting $\Pi'$ denote the corresponding fundamental group, the sequence of profinite groups
      \[
        1 \to \pi_1(A^\log \times_{\overline{A}^\log} \overline{a}^\log, a^\log)^\ell \to \pi_1(A^\log, a^\log)^\ell \to \Pi'^\ell \to 1
      \]
      is exact. Moreover, the profinite group $\pi_1(A^\log \times_{\overline{A}^\log} \overline{a}^\log, a^\log)^\ell \cong \pi_1(A \times_{\overline{A}} \overline{a}, a)^\ell$ is isomorphic to the maximal pro-$\ell$ quotient of the fundamental group of a smooth curve over an algebraically closed field of characteristic $0$.
      \item If $\sign{\eta_A}(n) = \textC$, then $A$ is finite \'{e}tale over $\overline{A}$. Let $\Pi'$ for the open subgroup of $\pi_1(\overline{A}^\log, \overline{a}^\log)$ corresponding to the strict k\'{e}t covering associated to the morphism $A \to \overline{A}$. Then the sequence of profinite groups
      \[
        1 \to \pi_1(A^\log \times_{\overline{A}^\log} \overline{a}^\log, a^\log)^\ell \to \pi_1(A^\log, a^\log)^\ell \to \Pi'^\ell \to 1
      \]
      is exact. Moreover, the profinite group $\pi_1(A^\log \times_{\overline{A}^\log} \overline{a}^\log, a^\log)^\ell$ is isomorphic to $\Int_\ell(1)$.
      \item If $\sign{\eta_A}(n) = \textN$, then $A$ is finite \'{e}tale over $\overline{A}$. Let $\Pi'$ for the open subgroup of $\pi_1(\overline{A}^\log, \overline{a}^\log)$ corresponding to the strict k\'{e}t covering associated to the morphism $A \to \overline{A}$. Then the sequence of profinite groups
      \[
        1 \to \pi_1(A^\log \times_{\overline{A}^\log} \overline{a}^\log, a^\log)^\ell \to \pi_1(A^\log, a^\log)^\ell \to \Pi'^\ell \to 1
      \]
      is exact. Moreover, the profinite group $\pi_1(A^\log \times_{\overline{A}^\log} \overline{a}^\log, a^\log)^\ell$ is isomorphic to $\Int_\ell(1)$.
    \end{enumerate}
    \item Assume that the polycurve $X$ is graphically inert. Then the natural morphism of profinite groups $\pi_1(A^\log, a^\log)^\ell \to \pi_1(\model{X}_n^\log, a^\log)^\ell$ is injective, and the sequence of profinite groups
    \[
      1 \to \pi_1(A^\log \times_{\overline{A}^\log} \overline{a}^\log, a^\log)^\ell \to \pi_1(A^\log, a^\log)^\ell \to \pi_1(\overline{A}^\log, \overline{a}^\log)^\ell \to 1
    \]
    is exact.
  \end{enumerate}
\end{prop}
...
\begin{lem}
  Let $K$ be a complete discrete valuation field, $X = X_n \to X_{n - 1} \to \cdots \to X_1 \to X_0 = \Spec \, K$ a hyperbolic polycurve over $K$. Assume that $X$ admits a compactified stable model $\model{X} = \model{X}_n \to \model{X}_{n - 1} \to \cdots \to \model{X}_1 \to \model{X}_0 = \Spec \, \mathcal{O}_K$ with respect to its parametrization. Let $\ell$ be a prime number which is invertible in $\mathcal{O}_K$. Assume that $X$ is pro-$\ell$-exact and graphically inert. Let $\mathfrak{A}$ be the stratification of $\model{X}_n$ obtained by applying the construction in Lemma \ref{StratificationOfStableCompactification} to the hyperbolic polycurve $X_n$. Let $A$ be a stratum of $\mathfrak{A}$. Then the following assertions hold:
  \begin{enumerate}
    \item the sequence of profinite groups
    \[
      1 \to I_A^\ell \to D_A^\ell \to \pi_1(A, a)^\ell \to 1
    \]
    is exact.
    \item the pro-$\ell$ inertia group $I_A^\ell$ of $A$ is isomorphic to a direct product of $r$ copies of $\Int_\ell(1)$, where $r$ is the log-rank of $A^\log$.
    \item $A$ has a natural structure of a hyperbolic polycurve over $k$, where $k$ is the residue field of $K$ as a discrete valuation field, and $A$ is pro-$\ell$-exact as a hyperbolic polycurve over $k$.
    \item if the dimension of $A$ is $n$, then $I_A^\ell$ is the center of $D_A^\ell$. このへんでちょっと調整がほしい
  \end{enumerate}
\end{lem}
\begin{proof}
  mada kaite nai
